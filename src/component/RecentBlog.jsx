    ...
    <>
        <div className={`px-5 lg:px-28  w-full ${theme === 'dark' ? 'bg-mysecondary' : 'bg-white'}`}>
            ...
            <div className="flex-col max-w-120">
                <div className='flex justify-between gap-8  w-full max-w-110 flex-wrap lg:flex-nowrap'>
                    <div className="w-full max-w-100 lg:w-2.5/5">
                        {blogs.length > 0 && (
                            <BlogCards key={blogs[0].id} blog={blogs[0]} first={'first'} />
                        )}
                    </div>
                    <div className="w-full max-w-100 flex flex-col gap-8 lg:w-2.5/5">
                        {blogs.slice(1,3).map((blog, index) => (
                            <div className="w-full max-w-100 lg:w-2.5/5" key={blog.id}>
