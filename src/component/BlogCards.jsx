...
export default function BlogCards({ blog , first , right ,under , all ,side , aa }) {
  ...
  return (
        <div  onClick={handleCardClick} className= {`${side ? ' w-full max-w-screen-sm md:w-full md:max-w-83 flex flex-col ' : ''} ${right ? 'flex justify-between gap-6 flex-wrap lg:flex-nowrap' : ''} ${under ? 'flex justify-between gap-8 pt-14 pb-8 flex-wrap lg:flex-nowrap' : ''} `}>
            <img className= {` w-full object-cover ${first ? 'mb-8 ' : ''} ${under ? ' w-full max-w-100 lg:w-2.5/5' : ''} `} src={blog.img} alt={''} ></img>
            <div className= {` w-full ${under ? ' max-w-100 lg:w-2.5/5' : ''} ${right ? 'w-72' : ''} `}>
              <div className="flex">
                ...
                <div className="flex gap-2">
                    ...
                    <button className={` ${first ? '  text-myfourth ' : ''} ${under ? "text-myfifth" : ''}  bg-mysix rounded-2xl	`}>{blog.btn2}</button>
                    <button className={`text-myfifth  py-0.5 px-2.5 bg-mysix rounded-2xl ${right ? 'hidden' : ''} ${under ? 'hidden' : ''}`} 	 >{blog.btn3}</button>
                </div>
            </div>
        </div>
  )
